...
if (opt$run_type == "gb") {
  ...
  # Prepare data_out for Excel output (if needed later)
  data_out <- out %>% 
    select(c(accession, pop_id = base_id, frag_id, primer_index, oligo, 
             left_oh, right_oh, frag_length))
  
  # Create the base sample info (nano) output with one row per group and required columns
  nano_out_base <- out %>%
    group_by(base_id) %>%
    mutate(
      CDS_length = nchar(chiseled_seq),  # compute string length per row
      Sample_ID = "",                    # placeholder for later assignment
      Fwindex = "",
      FwPrimer = "",
      Rvindex = "",
      RvPrimer = ""
    ...
    select(
      base_id,  # Ensure base_id is not dropped if needed later
      primer_index,
      Sample_ID,
  ...
  # Create version with 6 rows per original row:
  nano_out_6 <- nano_out_base[rep(1:nrow(nano_out_base), each = 6), ]
  nano_out_6 <- nano_out_6 %>% 
    group_by(primer_index) %>%
    mutate(row_num = rep(1:6, times = n() / 6)) %>%
    ungroup() %>%
    mutate(Sample_ID = paste0(primer_index, "_", row_num)) %>%
    select(-row_num)
  
  # Create version with 8 rows per original row:
  nano_out_8 <- nano_out_base[rep(1:nrow(nano_out_base), each = 8), ]
  nano_out_8 <- nano_out_8 %>% 
    group_by(primer_index) %>%
    mutate(row_num = rep(1:8, times = n() / 8)) %>%
    ungroup() %>%
    mutate(Sample_ID = paste0(primer_index, "_", row_num)) %>%
    select(-row_num)
  
  # Process seqs_out as needed (this part doesn't affect sample info file output)
  seqs_out <- out %>% 
    group_by(base_id) %>%
  ...
  if (opt$two_step == "on") {
    seqs_out <- out %>%
      left_join(two_step_data, by = "full_base_id", relationship = "many-to-many") %>%
      mutate(base_id = base_id.x) %>%
      group_by(base_id) %>%
      mutate(total_fragments = n()) %>%    
      select(c(accession, pop_id = base_id, primer_index, total_fragments, 
               n_fragments_in_set, internal_ohs, fidelity, f_primer, r_primer, 
               input_seq, chiseled_seq, second_step_assembly_seq)) %>%
      slice(1) %>%
      arrange(primer_index)
  }
  
  # Derive the output directory from opt$output_file
...
if (opt$run_type == "cds") {
  ...
  # Prepare the data_out for Excel output (if needed later)
  data_out <- out %>% 
    select(c(accession, pop_id = base_id, frag_id, primer_index, oligo, 
             left_oh, right_oh, frag_length))
  
  # Create the base nano output with one row per group
  nano_out_base <- out %>%
    group_by(base_id) %>%
    mutate(
      CDS_length = nchar(chiseled_seq),  # compute string length per row
  ...
  # Process seqs_out as needed (not affecting sample info file output)
  seqs_out <- out %>% 
    group_by(base_id) %>%
    select(c(accession, pop_id = base_id, primer_index, f_primer, r_primer, 
             total_fragments, n_fragments_in_set, internal_ohs, fidelity, 
             f_primer, r_primer, input_cai, chiseled_cai, input_seq, chiseled_seq)) %>%
    slice(1) %>% 
    arrange(primer_index)
...
# Construct file paths for other outputs
